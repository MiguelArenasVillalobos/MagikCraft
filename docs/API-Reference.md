...
- [Global variables](#global-variables)
  - [\_\_plugin variable](#__plugin-variable)
  - [server variable](#server-variable)
  - [self variable](#self-variable)
  - [config variable](#config-variable)
  - [events variable](#events-variable)
- [Module variables](#module-variables)
  - [\_\_filename variable](#__filename-variable)
  - [\_\_dirname variable](#__dirname-variable)
...
_ScriptCraft now uses the same module system as Node.js - see [Node.js Modules][njsmod] for more details._
...
_Modules in the `modules` directory are not automatically loaded at
startup_, instead, they are loaded and used by other modules/plugins
...
and should _never_ be used in modules. For example you can use it
here...
...
```javascript
events.on(Packages.net.canarymod.hook.player.BlockDestroyHook, function(
  evt,
  cancel
) {
...
To unregister a listener _outside_ of the listener function...
...
- location : _NB_ If a `Location` object is provided as a parameter, then it should be the only parameter.
...
The Drone object can be easily extended - new buidling recipes/blueprints can be added and can become part of a Drone's chain using the _static_ method `Drone.extend`.
...
a chain. So if you wanted to create a _grid_ of 20 houses ( 4 x 5 ),
...
Spheres are time-consuming to make. You _can_ make large spheres (250 radius) but expect the
...
file browser) then right-click and choose _Sharing Options_, check the
_Share this folder_ checkbox and the _Allow others to create and
delete files_ and _Guest access_ checkboxes. Click _Create Share_
...
exports.numberguess = function(player) {
  ...
  input(player, "Think of a number between 1 and 10 (q to quit)", function(
    guess,
    guesser,
    repeat
  ) {
...
events.projectileHit(function(event) {
  if (
    entities.arrow(event.projectile) && // it's an arrow
...
http.request("https://scriptcraftjs.org/sample.json", function(
  responseCode,
  responseBody
) {
...
slash(["time set 6000", "toggledownfall"], johnDoe);
```
...
_nicely_ by processing one item at a time then delaying processing of
...
2.  It uses ScriptCraft's `events` module to add a new _Event
    Handler_. An _Event Handler_ is a function that gets
    called whenever a particular _event_ happens in the game. The
...
Adding new _Event Handlers_ in ScriptCraft is relatively easy. Use one
...
function responsible for adding new _connection_ event handlers - that
is - functions which should be invoked when there's a new _connection_
event in the game. A new _connection_ event is fired whenever a player
