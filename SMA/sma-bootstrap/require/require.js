...
(function(rootDir, modulePaths, hooks, evaluate) {
    ...
    if (pkgJsonFile.exists()) {
      ...
      if (mainFile.exists()) {
        ...
        debug(
          "Found " +
            dir +
            "/package.json, but no entry point was specified or found for the module."
        );
        return null;
      }
    } else {
  ...
  When resolving module names to file paths, ScriptCraft uses the following rules...
   ...
   1. if the module does not begin with './' or '/' then ...
      ...
      1.3 Start node_modules resolution. If it's not there then
          Throw an Error.
    ...
    if (moduleName.match(/^[^\.\/]/)) {
      ...
      for (; i < modulePaths.length; i++) {
        ...
        if (resolvedFile.exists()) {
          ...
          // try appending a .js to the end
          resolvedFile = new File(modulePaths[i] + moduleName + "/index.js");
          debug("Resolved file: " + resolvedFile); // @DEBUG
      ...
      debug(
        parentDir +
          moduleName +
          " exists: " +
          new File(parentDir, moduleName).exists()
      );
      if ((file = new File(parentDir, moduleName)).exists()) {
        ...
        (file = new File(parentDir, moduleName + "/index.js")).exists()
      ) {
        // try index.js
        return file;
      }
